...
export const AdminPage: React.FC = () => {
  ...
  // Styles for team creation
  const teamInputStyle = "text-text-main";
  const teamInputBackground = "bg-blue-primary";

  const renderTeamForm = () => {
    return (
      <div>
        <input
          className={teamInputStyle}
          style={{ background: teamInputBackground }}
          placeholder="Team Name"
        />
        {/* Other inputs for team creation */}
      </div>
    );
  };
  ...
  const [newTeamPlayers, setNewTeamPlayers] = useState<
    { name: string; role: string; opgg: string }[]
  >([
    { name: "", role: "TOP", opgg: "" },
    { name: "", role: "JG", opgg: "" },
    { name: "", role: "MID", opgg: "" },
  ...
  const handleCreateTeam = async () => {
    ...
    if (validPlayers.length === 0)
      return showMessage("error", "Debe haber al menos un jugador");
  ...
  useEffect(() => {
    if (selectedMatch) {
      ...
      if (
        ...
        !selectedMatch.details.team1Players ||
        selectedMatch.details.team1Players.length === 0
      ) {
  ...
  const handleUpdateMatch = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/matches/${selectedMatch.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const [newPlayoffMatch, setNewPlayoffMatch] = useState({
    id: "",
    stage: "QF",
    team1Id: "",
    team2Id: "",
  });
  const [selectedPlayoffMatch, setSelectedPlayoffMatch] = useState<any | null>(
    null,
  );
  ...
  const handleUpdatePlayoffMatch = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/playoffs/${selectedPlayoffMatch.id}/winner`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/generatetoken`,
        {
          method: "POST",
  ...
  if (!token) {
    return (
      <div className="bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200 w-full max-w-md">
          ...
          {message && (
            <div
              className={`mb-4 p-3 rounded text-center text-sm font-bold ${message.type === "success" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
            >
              {message.text}
            </div>
          )}
  ...
  return (
    <div className="min-h-screen p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        {/* HEAD & AUTH */}
        <div className="p-4 rounded-sm shadow-sm bg-blue-header flex justify-between items-center border xs:border-2 border-text-dark">
          <h1 className="text-2xl font-bold text-text-main">Admin Dashboard</h1>
          <div className="flex items-center space-x-4">
            <button
              onClick={() => setToken("")}
              className="text-rose-600 border rounded-sm p-2 font-bold hover text-sm cursor-pointer hover:bg-rose-600 hover:text-text-main"
            >
              Cerrar Sesión
            </button>
          </div>
        ...
        {/* TABS */}
        <div className="grid grid-cols-3 border-b bg-blue-header border-slate-200">
        ...
        {message && (
          <div
            className={`p-4 rounded-lg font-bold text-center ${message.type === "success" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
          >
            {message.text}
          </div>
        )}
        ...
        {activeTab === "teams" && (
          <div className="space-y-6">
            ...
            <div className="bg-blue-header p-6 rounded-sm shadow-sm border xs:border-2 border-text-dark">
              <h2 className="text-lg font-bold text-text-main mb-4">
                Subir Logo / Imagen
              </h2>
              <div className="flex gap-6 items-start">
                <div
                  className={`flex-1 border-2 border-dashed h-[120px] rounded-sm p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-colors ${
                    uploadStatus === "uploading"
                      ? "border-blue-400"
                      : "border-slate-300 hover:border-gold hover:bg-gold/10"
                  }`}
                  ...
                  onClick={() =>
                    document.getElementById("fileInputTeams")?.click()
                  }
                >
                  <input
                    type="file"
                  ...
                  {uploadStatus === "uploading" ? (
                    ...
                    <>
                      <p className="font-bold text-text-dark border-text-dark text-sm">
                ...
                {lastUploadedUrl && (
                  <div className="flex-1 rounded-sm border xs:border-2 border-text-dark p-4 flex items-center justify-center text-center transition-colors h-[120px] gap-4">
                    <img
                      src={lastUploadedUrl}
                      alt="Preview"
                      className="w-16 h-16 object-cover rounded bg-white border border-text-dark"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="bg-white border border-slate-200 rounded px-2 py-1 text-xs w-full text-slate-600 font-mono"
                        />
                      </div>
                      <button
                        onClick={() => {
                        ...
                        className="text-xs text-text-main hover:bg-blue-600 uppercase font-bold mt-2 border xs:border-2 w-full p-2 cursor-pointer transition-all duration-300"
                      >
            ...
            <div className="bg-blue-header p-6 rounded-sm shadow-sm border xs:border-2 border-text-dark">
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={newTeam.seed}
                    onChange={(e) =>
                      setNewTeam({ ...newTeam, seed: parseInt(e.target.value) })
                    }
                  />
                </div>
                <div>
                ...
                <div className="col-span-2 ">
                  ...
                  <div className="text-xs text-slate-400 text-right">
                    {newTeam.description?.length || 0}/100
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="mt-8 cursor-pointer bg-gold text-blue-primary px-6 py-2 rounded-lg font-bold hover:bg-gold/80 transition-colors w-full"
              >
                Crear Equipo
              </button>
            </div>
            ...
            <div className="bg-blue-header p-6 rounded-sm shadow-sm border xs:border-2 border-text-dark">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {teams.map((t) => (
                  <div
                    key={t.id}
                    className="p-4 border xs:border-2 border-text-dark rounded-sm flex items-center space-x-4"
                  >
                    <img
                      src={t.logo}
                      className="w-10 h-10 object-cover border-black border-2 rounded-full"
                      alt=""
                    />
                    <div>
                      <p className="font-bold text-text-main">{t.name}</p>
                      <p className="text-sm text-text-dark uppercase font-bold">
                        Seed: {t.seed} • ID: {t.id}
                      </p>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-blue-header p-6 rounded-sm shadow-sm border xs:border-2 border-text-dark">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="label-std text-text-main">Match ID</label>
                    <input
                      placeholder="ej: swiss-1-1"
                      className="input-std w-full bg-blue-secondary p-2 rounded-sm text-text-main mt-2"
                      value={newMatch.id}
                      onChange={(e) =>
                        setNewMatch({ ...newMatch, id: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    <label className="label-std text-text-main">Ronda</label>
                    <input
                      placeholder="Num"
                      type="number"
                      className="input-std w-full bg-blue-secondary p-2 rounded-sm text-text-main mt-2"
                  ...
                  <div>
                    <label className="label-std text-text-main">Equipo 1</label>
                    <select
                      className="input-std w-full bg-blue-secondary p-2 rounded-sm text-text-main mt-2"
                  ...
                  <div>
                    <label className="label-std text-text-main">Equipo 2</label>
                    <select
                      className="input-std w-full bg-blue-secondary p-2 rounded-sm text-text-main mt-2"
                  ...
                  <button
                    ...
                    className="mt-4 cursor-pointer bg-gold text-blue-primary px-6 py-2 rounded-lg font-bold hover:bg-gold/80 transition-colors w-full"
                  >
                    Crear
                  </button>
                </div>
              ...
              {/* MATCH LIST */}
              <div className="bg-blue-header p-6 rounded-sm shadow-sm border xs:border-2 border-text-dark h-[600px] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-lg font-bold text-text-main">Partidos</h2>
                  <button onClick={fetchMatches} className="cursor-pointer">
                    <RefreshCw className="w-4 h-4 text-text-main" />
                  </button>
                </div>
                  ...
                  {matches
                    ...
                    .map((m) => (
                      <div
                        ...
                        className={`p-3 border rounded-lg cursor-pointer hover:bg-blue-secondary transition-colors ${selectedMatch?.id === m.id ? "border-gold bg-gold/10" : "border-slate-200"}`}
                      >
                        <div className="grid grid-cols-2 justify-between text-xs font-bold text-text-main mb-1">
                          <span className="uppercase text-left">
                            Ronda {m.round}
                          </span>
                          <span
                            className={`uppercase text-right ${m.status === "completed" ? "text-emerald-600" : "text-amber-600"}`}
                          >
                            {m.status}
                          </span>
                        </div>
                        <div className="font-bold text-sm text-text-main grid grid-cols-[1fr_1fr_1fr] gap-2 justify-between items-center">
                          ...
                          {Array(5)
                            ...
                            .map((_, i) => (
                              <input
                                ...
                                onChange={(e) => {
                                  const newBans = [
                                    ...matchUpdateData.details.team1Bans,
                                  ];
                          ...
                          {Array(5)
                            ...
                            .map((_, i) => (
                              <input
                                ...
                                onChange={(e) => {
                                  const newBans = [
                                    ...matchUpdateData.details.team2Bans,
                                  ];
                                  newBans[i] = e.target.value;
                                  setMatchUpdateData({
                                    ...matchUpdateData,
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <h2 className="text-lg font-bold text-slate-800 mb-4">
                  Crear Partido Playoff
                </h2>
                <div className="space-y-4">
                  <div>
                    <label className="label-std">Match ID</label>
                    <input
                      placeholder="Match ID (ej: QF-1)"
                      className="input-std w-full"
                      value={newPlayoffMatch.id}
                      onChange={(e) =>
                        setNewPlayoffMatch({
                          ...newPlayoffMatch,
                          id: e.target.value,
                  ...
                  <button
                    onClick={handleCreatePlayoffMatch}
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-[600px] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={fetchPlayoffMatches}>
                  ...
                  {playoffMatches.map((m) => (
                    ...
                    >
                      <div className="flex justify-between text-xs font-bold text-text-dark mb-1">
                        <span>{m.stage}</span>
                        <span
                          className={`${m.status === "completed" ? "text-emerald-600" : "text-amber-600"}`}
                        >
                          {m.status}
                        </span>
                      </div>
            ...
            <div className="lg:col-span-8">
              {selectedPlayoffMatch ? (
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-6 max-h-screen overflow-y-auto">
                  ...
                  <div className="mb-6">
                    <label className="label-std">
                      Winner ID (Series Winner)
                    </label>
                    <select
                      className="input-std w-full"
                      value={playoffWinnerId}
                      onChange={(e) => setPlayoffWinnerId(e.target.value)}
                    >
                      <option value="">Select Series Winner</option>
                      <option value={selectedPlayoffMatch.team1Id}>
                        {
                          teams.find(
                            (t) => t.id === selectedPlayoffMatch.team1Id,
                          )?.name
                        }
                      </option>
                      <option value={selectedPlayoffMatch.team2Id}>
                        {
                          teams.find(
                            (t) => t.id === selectedPlayoffMatch.team2Id,
                          )?.name
                        }
                      </option>
                    </select>
                  ...
                  <div className="mt-4">
                    <button
                      onClick={() =>
                        setPlayoffGames([
                          ...playoffGames,
                          createEmptyGame(playoffGames.length + 1),
                        ])
                      }
                  ...
                  <div className="mt-8 flex justify-end">
                    <button
                      onClick={handleUpdatePlayoffMatch}
                      className="btn-primary flex items-center gap-2"
                    >
                      <Save className="w-5 h-5" />
                      Guardar Serie
